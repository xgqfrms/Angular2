...
以下是其他重要的Angular功能和服务的简要，按字母顺序排列的列表。大多数都包括在本文档中（或很快就会）
    ...
    动画：动画组件行为没有深入的动画技术知识或CSS，用Angular的动画库。

    https://angular.cn/docs/ts/latest/guide/animations.html

    变化检测：变化检测文档将介绍Angular如何决定组件属性值已更改，何时更新屏幕，以及如何使用区域拦截异步活动并运行其更改检测策略。

    事件：事件文档将介绍如何使用组件和服务通过发布和订阅事件的机制来引发事件。

    表单：支持基于HTML的验证和脏检查的复杂数据输入方案.

    HTTP：与服务器通信以使用HTTP客户端获取数据，保存数据和调用服务器端操作。

    生命周期钩子：通过实现生命周期钩子接口，在组件的生命周期中，从创建到销毁生命周期钩子。

    管道：在模板中使用管道，通过转换显示值来改善用户体验。考虑这个currency管道表达式：

    price | currency:'USD':true 
    它显示“42.33”的价格为$ 42.33。

    路由器：在客户端应用程序内从页面到页面，并且从不离开浏览器。

    测试：当应用程序部件使用Angular测试平台与Angular框架交互时，对其运行单元测试。
